...
function indexMaker (dictionaryFileName, wordsFileName, syllablesFileName) {
  let termins = fs.readFileSync(dictionaryFileName + '.csv').toString().split("\n");
  ...
  fs.writeFileSync('./files/' + wordsFileName + '.json', words);

  let syllables = JSON.stringify(indexSyllables);
  fs.writeFileSync('./files/' + syllablesFileName + '.json', syllables);
}
